...
async function get_project_idea(user_id, languages, number_of_ideas = 1) {
    const user = await user_service.getById(user_id);
    if (!user) {
        throw new Error(`User id ${user_id} does not exist`);
    }

    if (!Array.isArray(languages) || languages.length === 0 || !languages.every(lang => typeof lang === 'string' && lang.trim() !== '')) {
        throw new Error(`languages field is required and must be a non-empty array of valid strings`);
    }
    ...
    try {
        ...
        user.recommendations.projects = user.recommendations.projects || [];
